    ...
    for (const ev of events) {
      switch (ev.kind) {
        ...
        case 'abilityCast': {
          ...
          const color = {
            bannerfall: 0xffdf70,
            shieldbreaker: 0x73c7ff,
            lionCharge: 0xff8a52,
            crownstorm: 0xd88cff,
          }[ev.abilityId] ?? 0xffdf70;
          ...
          this.flashes.push({ gfx, startTick: tick, ticks: 12, color });
          this.spawnFlash(p.x, p.y - 8, tick, color, ev.abilityId === 'crownstorm' ? 22 : 14);
          break;
        }
        default:
          break;
