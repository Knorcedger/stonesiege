...
function frameFor(name: string): TestFrame {
  return name.startsWith('bld/') ? BUILDING_FRAME : UNIT_FRAME;
}

/** A GameAssets stand-in that resolves every name without an atlas or a canvas. */
export const fakeAssets = {
  tryResolve: (name: string) => frameFor(name),
  resolveFrame: (name: string) => frameFor(name),
  // Every name resolves here, so the first candidate always wins — same as the
  // real resolver, which only falls through when a frame is genuinely absent.
  resolveFirst: (candidates: readonly string[]) => {
    const name = candidates[0] ?? '';
    return { frame: frameFor(name), name };
  },
  contentTopPx: () => 0,
  frameCount: () => 4,
