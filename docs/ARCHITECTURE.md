...
- **Economy speed**: a deterministic 1×/2×/4× match setting scales active construction,
  stationary resource gathering, and the shared training/research queue. Changes are commands,
  so replays and snapshots reproduce them without altering movement, combat, or visual animation
  timing.
